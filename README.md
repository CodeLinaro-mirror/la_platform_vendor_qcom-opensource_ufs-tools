...
This is the downstream repo of ufs-tools. This code base is initially copied from the upstream repo
https://github.com/quic/ufs-tools. Having this downstream repo is mainly to serve three purposes-

1. We need a downstream repo from which Flashval can find the latest ufs-tools to make release.
2. We need a downstream repo to maintain downstream changes, e.g., json report generate.
3. We need a downstream repo to maintain quick fixes which may not be suitable for upstream.

upstream (github) <---> downstream (gitlab) ---> LA SP
